...
/* config.h for user settings. */
#include "config.h"
#include "keys.h" /* Kbd compat with the linux world. */
...
void selnotify(XEvent *e) {
    ...
    do {
        if (XGetWindowProperty(xw.dpy, xw.win, property, ofs, BUFSIZ / 4, False, AnyPropertyType,
...
void xclear(int x1, int y1, int x2, int y2) {
    XftDrawRect(xw.draw, &dc.col[IS_SET(MODE_REVERSE) ? defaultfg : defaultbg], x1, y1, x2 - x1,
                y2 - y1);
}
...
int xloadfont(Font *f, FcPattern *pattern) {
    ...
    XftTextExtentsUtf8(xw.dpy, f->match, (const FcChar8 *)ascii_printable, strlen(ascii_printable),
                       &extents);
...
void xinit(int cols, int rows) {
    ...
    xw.win = XCreateWindow(
        xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, xw.depth, InputOutput,
...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int dmode) {
    ...
    r.y = 0;
    r.height = win.ch;
    r.width = width + 10; /* Allow for wide characters. */
    XftDrawSetClipRectangles(xw.draw, winx, winy, &r, 1);

    if (dmode & DRAW_BG) {
        /* Intelligent cleaning of the borders. */
        if (x == 0) {
            xclear(0, (y == 0)? 0 : winy, win.vborderpx, winy + win.ch + 
    ...
    if (dmode & DRAW_FG) {
        ...
        if (base.mode & ATTR_UNDERLINE) {
            // {{{ Underline/Undercurl drawing.
            // Underline Color
            const int widthThreshold = 28;           // +1 width every widthThreshold px of font
            int wlw = (win.ch / widthThreshold) + 1; // Wave Line Width
            int linecolor;
            if ((base.ucolor[0] >= 0) && !(base.mode & ATTR_BLINK && win.mode & MODE_BLINK) &&
            ...
            if (base.ustyle != 3) {
                ...
                // Last wave tail
                arcs[i * 2 + 1] =
                    (XArc){wx + ww * i + ww * 0.75, wy, ceil(ww / 2.), wh, 180 * 64, 90 * 64};
                // First wave tail
                i++;
                ...
                */
                ...
                if (npoints >= 3) {
                    ...
                    for (int i = 2; i < npoints - 1; i++) {
                        points[i] =
                            (XPoint){.x = ww / 2, .y = (isSlopeRising(wx, i, ww)) ? wh / 2 : -wh / 2};
                        waveLength += ww / 2;
                    }

                    // Last point
                    points[npoints - 1] = (XPoint){
                        .x = ww / 2, .y = (isSlopeRising(wx, npoints - 1, ww)) ? wh / 2 : -wh / 2};
                    waveLength += ww / 2;
                    ...
                    if (waveLength < width) { // Add a bonus point?
                        ...
                        points[npoints] = (XPoint){
                            .x = marginEnd,
                            .y = (isSlopeRising(wx, npoints, ww)) ? (marginEnd) : (-marginEnd)};
        ...
        // End drawing of underline/undercurl }}}
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len) {
    ...
    if (IS_SET(MODE_FOCUSED)) {
        ...
        XftDrawRect(xw.draw, &drawcol, win.hborderpx + cx * win.cw, win.vborderpx + cy * win.ch,
                    win.cw - 1, 1);
        XftDrawRect(xw.draw, &drawcol, win.hborderpx + cx * win.cw, win.vborderpx + cy * win.ch, 1,
                    win.ch - 1);
...
int xsetcursor(int cursor) {
    ...
    win.cursor = cursor;
    cursorblinks =
        win.cursor == 0 || win.cursor == 1 || win.cursor == 3 || win.cursor == 5 || win.cursor == 7;
    return 0;
}
